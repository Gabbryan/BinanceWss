...
class SparkController:
    def __init__(self, app_name="DataAggregatorSpark", configs=None, cache_session=False):
        ...
        self.logger = LoggingController(self.app_name)  # Utilisation de LoggingController
    ...
    def start_session(self):
        if self.spark is None:
            try:
                ...
                self.logger.log_info("Spark session started.")
            except Exception as e:
                self.logger.log_error(f"Failed to start Spark session: {e}")
                raise
        else:
            self.logger.log_warning("Spark session is already active.")
    ...
    def stop_session(self):
        if self.spark is not None and not self.cache_session:
            try:
                self.spark.stop()
                self.spark = None
                self.logger.log_info("Spark session stopped.")
            except Exception as e:
                self.logger.log_error(f"Failed to stop Spark session: {e}")
                raise
        elif self.spark is None:
            self.logger.log_warning("Spark session is not active.")
    ...
    def with_spark(self, func, *args, **kwargs):
        with self.session() as spark:
            try:
                ...
                self.logger.log_error(f"Error running function with Spark: {e}")
                raise
    ...
    def repartition_df(self, df, num_partitions):
        try:
            ...
            self.logger.log_error(f"Error repartitioning DataFrame: {e}")
            raise
    ...
    def cache_df(self, df):
        try:
            ...
            self.logger.log_error(f"Error caching DataFrame: {e}")
            raise
    ...
    def checkpoint_df(self, df, path, is_eager=True):
        try:
            ...
            self.logger.log_error(f"Error checkpointing DataFrame: {e}")
            raise
    ...
    def broadcast_variable(self, value):
        try:
            ...
            self.logger.log_error(f"Error broadcasting variable: {e}")
            raise
    ...
    def create_accumulator(self, value, accum_type):
        try:
            ...
            self.logger.log_error(f"Error creating accumulator: {e}")
            raise
    ...
    def read_data(self, path, format="parquet", **options):
        try:
            ...
            self.logger.log_error(f"Error reading data: {e}")
            raise
    ...
    def write_data(self, df, path, format="parquet", mode="overwrite", **options):
        try:
            ...
            self.logger.log_error(f"Error writing data: {e}")
            raise
