...
class DataProcessor:
    def __init__(self):
        ...
        self.client = bigtable.Client(project=BIGTABLE_PROJECT_ID, admin=True)
        self.instance = self.client.instance(BIGTABLE_INSTANCE_ID)
        self.table = self.instance.table(BIGTABLE_TABLE_ID)
    ...
    def save_dataframe_to_bigtable(
        self, df, symbol, exchange, timeframe, start_date, end_date
    ):
        def create_mutation(row_key, cells):
            direct_row = row.DirectRow(row_key)
            for column, value in cells:
                direct_row.set_cell("cf1", column, value)
            return direct_row

        mutations = []
        for row in df.collect():
            row_key = f"{exchange}#{symbol}#{timeframe}#{row['time_rounded']}".encode()
            cells = [
                ("open", str(row["open"]).encode()),
                ("close", str(row["close"]).encode()),
                ("high", str(row["high"]).encode()),
                ("low", str(row["low"]).encode()),
                ("qty", str(row["qty"]).encode()),
                ("footprint", str(row["footprint"]).encode()),
            ]
            mutation = create_mutation(row_key, cells)
            mutations.append(mutation)

            if len(mutations) >= 100000:
                self.table.mutate_rows(mutations)
                mutations = []

        if mutations:
            self.table.mutate_rows(mutations)

        logger.info(f"DataFrame has been uploaded directly to Bigtable for {symbol}")
        return f"Bigtable path: {exchange}#{symbol}#{timeframe}"
    ...
    def data_pipeline_exec(
        ...
        logger.info("Saving data on Bigtable")
        self.save_dataframe_to_bigtable(
