...
@slack_decorators.notify_with_result(
    header="Cicada has exported a new daily aggTrades file", color="#6a0dad"
)
def save_trades_to_csv(trades, directory, now):
    ...
    return f"The Cicada purge has commenced. Trades saved to `{filename}`."


# Decorated function to delete trades
@slack_decorators.notify_with_result(
    header="Daily batch delete operation on Cicada", color="#6a0dad"
)
def delete_trades(delete_url, headers, trade_ids):
    ...
    return f"The Cicada database has been cleansed. *{len(trade_ids)}* trades have been purged."
...
def wait_until_next_run():
    while True:
        ...
        slack_channel.send_message(
            " 🛠 Next run scheduled to purge Cicada database 🛠",
