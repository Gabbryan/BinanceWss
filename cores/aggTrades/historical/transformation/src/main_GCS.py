...
log_file_path = os.getenv("LOG_FILE_PATH", "/Trustia/Cicada-binance/cores/aggTrades/historical/transformation/src/app.log")
...
def process_year_data(
    ...
    try:
        ...
        gcs_module.upload_to_gcs(df_joined_final, 
                                 symbol,
                                 timeframe, 
                                 year,
                                 start_month,  
        ...
        for timeframe in timeframes:
            ...
            for year in gcs_paths_by_year.keys():
                try:
                    ...
                    year_success_path = f"transformed/Binance/{crypto.replace('/', '')}/aggTrades_historical/{timeframe}/{year}_{start_month:02d}_{end_month:02d}/data.parquet"
                    success_files = gcs_module.list_gcs_success_files(year_success_path)
